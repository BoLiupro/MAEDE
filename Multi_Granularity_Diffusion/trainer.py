        ...
        for epoch_no in range(self.epochs):
            ...
            with tqdm(train_iter, total=total) as it:
                for batch_no, data_entry in enumerate(it, start=1):
                    ...
                    if self.log_metrics:
                        wandb.log({'train/loss': avg_epoch_loss}, step=self.total_step)
            ...
            # 保存 checkpoint
            # checkpoint_path = os.path.join(self.checkpoint_dir, f"model_epoch_{epoch_no+1}.pth")
            # torch.save(net.state_dict(), checkpoint_path)
            # print(f"Checkpoint saved at {checkpoint_path}")
            torch.save(net, self.checkpoint_dir+f"/net_{epoch_no}.pth")
            ...
            if validation_iter is not None:
                ...
                with tqdm(validation_iter, total=total, colour="green") as it:
                    for batch_no, data_entry in enumerate(it, start=1):
                        ...
                        if self.log_metrics:
                            wandb.log({'val/loss': avg_epoch_loss_val}, step=self.total_step)
                        if self.num_batches_per_epoch == batch_no:
                            break
                    it.close()
                    if self.log_metrics:
