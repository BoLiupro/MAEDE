...
class mgtsdTrainingNetwork(nn.Module):
    ...
    def unroll_encoder(
        ...
        future_time_feat: Optional[torch.Tensor], # prediction network 不需要
        future_target_cdf: Optional[torch.Tensor], # prediciton network 不需要
        target_dimension_indicator: torch.Tensor,
    ) -> Tuple[
        torch.Tensor,
        Union[List[torch.Tensor], torch.Tensor],
        ...
        for i in range(self.num_gran):
            ...
            outputs.append(output) # 结果
            states.append(state) # 记录上一个隐藏态
    ...
    def forward(
        ...
        hidden_state_4: torch.Tensor,
        hidden_state_8: torch.Tensor,
    ) -> Tuple[torch.Tensor, ...]:
        """
        ...
        hidden_state = [hidden_state_1[:,-seq_len:,:],hidden_state_2[:,-seq_len:,:],hidden_state_4[:,-seq_len:,:],hidden_state_8[:,-seq_len:,:]] # [3,B,216,128]
        if isinstance(hidden_state, list):
            hidden_state = torch.stack(hidden_state, dim=0)  # 变成 shape: (3,batch_size, seq_len, hidden_size)

        # # unroll the decoder in "training mode", i.e. by providing future data
        # rnn_outputs, _, scale, _, _ = self.unroll_encoder(
        #     past_time_feat=past_time_feat, # [N,T,F]=[128,192,4]
        #     past_target_cdf=past_target_cdf, # [N,T,num_gran]=[128,192,2]
        #     past_observed_values=past_observed_values,
        #     past_is_pad=past_is_pad,
        #     future_time_feat=future_time_feat, #[N,T,F]=[128,24,4]
        #     future_target_cdf=future_target_cdf,
        #     target_dimension_indicator=target_dimension_indicator,
        # )# [num_gran,B,context_len+prediction_len,num_cell] = [3,32,24+3,256]
        ...
        # rnn_outputs_2 = rnn_outputs  # outputs from multiple rnns [3,B,c+p,num_cell]=[3,32,27,256]

        """目标在这里替换！！！"""
        # rnn_outputs_2 = hidden_state # [num_gran,B,context_length+prediction_length,F] =[3,32,24+3,640]
        rnn_outputs_2 = hidden_state[:self.num_gran] # 指定粒度
        rnn_outputs_2 = rnn_outputs_2.to(torch.float32)  # 转换为 float32
        ...
        for ratio_index, share_ratio in enumerate(self.share_ratio_list):
            cur_likelihood = self.diffusion.log_prob(targets[ratio_index], distr_args[ratio_index],share_ratio=share_ratio).unsqueeze(-1)
...
class mgtsdPredictionNetwork(mgtsdTrainingNetwork):
    ...
    def sampling_decoder(
        ...
        repeat_hidden_state = repeat(hidden_state,dim=1) #[3,100,195,640]
        ...
        for k in range(self.prediction_length):  # future samples from multi-gran
            for m in range(self.num_gran):
                ...
                #     unroll_length=1, # 每一次预测的时间步
                # )
                """自己的代码改过来"""
                rnn_outputs = repeat_hidden_state[m,:,k:k+1,:].to(torch.float32)  # 转换为 float32
    ...
    def forward(
        ...
        if isinstance(hidden_state_1, list):
            hidden_state_1 = torch.stack(hidden_state_1, dim=0)  # (batch_size, seq_len, hidden_size)
            hidden_state_2 = torch.stack(hidden_state_2, dim=0)  # (batch_size, seq_len, hidden_size)
            hidden_state_4 = torch.stack(hidden_state_4, dim=0)  # (batch_size, seq_len, hidden_size)
            hidden_state_8 = torch.stack(hidden_state_8, dim=0)  # (batch_size, seq_len, hidden_size)
        # 确保 hidden_state_4 和 hidden_state_8 形状是 (batch_size, seq_len, hidden_size)
        if hidden_state_1.dim() == 2:  # 说明缺少 batch 维度
            hidden_state_1 = hidden_state_1.unsqueeze(0)  # 变成 (1, seq_len, hidden_size)
            hidden_state_2 = hidden_state_2.unsqueeze(0)  # 变成 (1, seq_len, hidden_size)
            hidden_state_4 = hidden_state_4.unsqueeze(0)  # 变成 (1, seq_len, hidden_size)
            hidden_state_8 = hidden_state_8.unsqueeze(0)  # 变成 (1, seq_len, hidden_size)

        # 使用 torch.cat 进行拼接，最终形状应该是 (3，batch_size, seq_len, hidden_size)
        hidden_state = torch.cat([hidden_state_1, hidden_state_2, hidden_state_4,hidden_state_8], dim=0)
        hidden_state = hidden_state.unsqueeze(1) #[3,1,seq_len,hidden_size]
        ...
        # _, begin_states, _, _, _ = self.unroll_encoder(
        ...
        return self.sampling_decoder(
            ...
            hidden_state= hidden_state[:self.num_gran]
        )
...
#         seq_len = self.context_length + self.prediction_length
#         # hidden_state = [hidden_state_1[:,-seq_len:,:],hidden_state_4[:,-seq_len:,:],hidden_state_8[:,-seq_len:,:]] # [3,B,216,128]
#         # if isinstance(hidden_state, list):
#         #     hidden_state = torch.stack(hidden_state, dim=0)  # 变成 shape: (3,batch_size, seq_len, hidden_size)
...
#             repeated_states_list = [repeat(s, dim=1) for s in begin_states]
#         else:
#             repeated_states_list = [repeat(begin_state, dim=1) for begin_state in begin_states]
#         # for each future time-units we draw new samples for this time-unit
#         # and update the state
#         future_samples_list = [[] for _ in range(self.num_gran)]
#         for k in range(self.prediction_length):  # future samples from multi-gran
...
#         # # 使用 torch.cat 进行拼接，最终形状应该是 (3，batch_size, seq_len, hidden_size)
#         # hidden_state = torch.cat([hidden_state_1, hidden_state_4, hidden_state_8], dim=0)
#         # hidden_state = hidden_state.unsqueeze(1) #[3,1,seq_len,hidden_size]
