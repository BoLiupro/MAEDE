...
from Multi_Granularity_Diffusion.model.feature import fourier_time_features_from_frequency, lags_for_fourier_time_features_from_frequency
from Multi_Granularity_Diffusion.model.estimator import PyTorchEstimator
...
class mgtsdEstimator(PyTorchEstimator):
    def __init__(
        ...
        **kwargs,
        ...
        ) # time_feature产生,两个freq

        # context_length + max(lags_seq) 24/12 + 168 = 192/180
    ...
    def create_transformation(self) -> Transformation:
        ...
        return Chain(
            [
                ...
                AsNumpyArray(field=FieldName.HIDDEN_STATE_1,
                             expected_ndim=2),  
                AsNumpyArray(field=FieldName.HIDDEN_STATE_2,
                             expected_ndim=2),  
                AsNumpyArray(field=FieldName.HIDDEN_STATE_4,
                             expected_ndim=2),  
                AsNumpyArray(field=FieldName.HIDDEN_STATE_8,
                             expected_ndim=2),  
            ]
        )
    ...
    def create_instance_splitter(self, mode: str) -> InstanceSplitter:
        ...
        return InstanceSplitter(
            ...
            hidden_state_1=FieldName.HIDDEN_STATE_1,
            hidden_state_2=FieldName.HIDDEN_STATE_2,  # 🟢 传递 HIDDEN_STATE
            hidden_state_4=FieldName.HIDDEN_STATE_4,  # 🟢 传递 HIDDEN_STATE
            hidden_state_8=FieldName.HIDDEN_STATE_8,  # 🟢 传递 HIDDEN_STATE
        ) + (
            RenameFields(
                {
    ...
    def get_metric(self, transformation, trained_net, device, data_test, prefix=""):
        ...
        targets_fine.append(targets[0].iloc[:,:100])
        # for i in range(7):
        #     targets_fine.append(targets[i].iloc[:, :100])
        ...
        # for day in range(7):
        #     forecast_fine[day].samples = forecasts[day].samples[:, :, :100]
        forecast_fine[0].samples = forecasts[0].samples[:, :, :100] # sample [采样次数,预测步长,目标维度]
        import warnings
        # Ignore all warnings
